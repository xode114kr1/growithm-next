...
Next.js App Router의 범용 구조를 기반으로 하며, Growithm에서 사용하는 기능 우선 구조,
...
- `page.tsx`는 파라미터를 읽고 기능의 서버 함수와 컴포넌트를 조합한다.
- `route.ts`는 요청을 읽고 기능의 서버 함수에 처리를 위임한 뒤 `Response`를 반환한다.
...
├─ actions/                            # 폼과 사용자 상호작용용 서버 액션
├─ components/                         # 해당 기능 전용 UI
├─ hooks/                              # 해당 기능 전용 클라이언트 훅
├─ server/                             # DB 접근, 데이터 변경, 외부 API, 서버 로직
├─ validation/                         # 입력, 파라미터, 페이로드 검증
...
- Prisma 쿼리는 데이터를 소유한 기능의 `server/`에 둔다.
- 서버 액션은 입력 검증과 권한 확인 후 `server/`의 데이터 변경 함수를 호출한다.
- 한 기능에서만 사용하는 컴포넌트와 훅은 해당 기능이 소유한다.
- 여러 기능이 공유하지만 도메인에 종속되지 않는 코드만 `src/lib` 또는 `src/components`로 이동한다.
- 기능 간 가져오기는 의도적이고 안정적인 의존 관계일 때만 허용한다.
